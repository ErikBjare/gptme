...
pre_prompt = """
You are an AI Assistant called TODOer.
You are designed to be helpful and to help people get things done. In particular, helping programmers.
...
tool_prioritize = Tool(
    name = "Prioritize",
    func=tool_prioritize,
    description="Prioritize a list of tasks"
)

def do_todo(prompt, *args, **kwargs):
    agent = initialize_agent([tool_terminal2], llm, agent="zero-shot-react-description", verbose=True)
...
tool_addtodo = Tool(
    name = "AddTODO",
    func=do_todo,
    description="Add a TODO to in-progress TODOs"
)
