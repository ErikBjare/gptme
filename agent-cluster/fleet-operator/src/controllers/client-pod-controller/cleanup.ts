 ...
 */
export async function cleanupInactivePods(this: any) {
  try {
    const clientPodList = await this.k8sClient.listClientPods() as { items: ClientPod[] };
    const now = new Date();
    ...
    for (const clientPod of clientPodList.items) {
      ...
      if (elapsedSeconds > timeoutSeconds) {
        logger.info(`ClientPod ${clientPod.metadata.name} has timed out, cleaning up`);
 ...
 */
export function startCleanupTimer(this: any, intervalMinutes: number = 5) {
  // Run cleanup every intervalMinutes
  setInterval(() => {
    this.cleanupInactivePods().catch((err: any) => {
