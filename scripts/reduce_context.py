...
def process_conversation(
    ...
    with open(input_file, "rb") as f:
        try:
            # Try to parse as standard TOML first
            data = tomllib.load(f)
            messages = data.get("messages", [])
        except Exception:
            # If that fails, try to read as text and manually fix the structure
            f.seek(0)
            content = f.read().decode("utf-8")

            # Convert to proper TOML format by wrapping in a messages array
            proper_toml = "messages = [\n" + content + "\n]"

            try:
                data = tomllib.loads(proper_toml.encode("utf-8"))
                messages = data.get("messages", [])
            except Exception as e:
                # If still failing, try one more approach - read line by line
                print(f"Error parsing TOML: {e}")
                print("Trying line-by-line parsing...")

                # Read the file as text and split into message blocks
                with open(input_file, encoding="utf-8") as text_f:
                    content = text_f.read()

                # Record original metrics for reporting
                original_size = len(content)

                # Use regex to find all message blocks
                message_blocks = re.findall(
                    r"\[\[messages\]\](.*?)(?=\[\[messages\]\]|$)", content, re.DOTALL
                )
                ...
                for block in message_blocks:
                    # Extract role and content with regex
                    role_match = re.search(r'role = "([^"]*)"', block)
                    role = role_match.group(1) if role_match else "unknown"

                    content_match = re.search(
                        r'content = """(.*?)"""', block, re.DOTALL
                    )
                    if not content_match:
                        content_match = re.search(r'content = "([^"]*)"', block)

                    msg_content = content_match.group(1) if content_match else ""

                    # Process the content
                    processed_msg_content = process_message_content(role, msg_content)

                    # Replace the content in the block
                    if content_match:
                        if '"""' in block:
                            processed_block = block.replace(
                                msg_content, processed_msg_content
                            )
                        else:
                            processed_block = block.replace(
                                f'"{msg_content}"', f'"""{processed_msg_content}"""'
                            )
                    else:
                        processed_block = block

                    processed_content += "[[messages]]" + processed_block

                    # Track message size for reporting
                    size = len(processed_block)
                    preview = processed_msg_content.replace("\n", " ")[:100]
                    if len(preview) == 100:
                        preview += "..."
                ...
                if top_messages > 0:
                    ...
                    for i, (size, role, preview) in enumerate(
                        message_sizes[:top_messages]
                    ):
                ...
                # Failed to parse with all methods
                print("Failed to parse conversation file with all methods.")
                return None
